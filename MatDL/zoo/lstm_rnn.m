function [out, grads, opt] = lstm_rnn(X, M,model, y, opt)
    ...
    [scores, cache3] = forward(d2(:, :, end), wy, by);
     out = scores;
%      out = softmax_forward(scores);  % obtain the output from outputlayer
    %[out, cache] =  relu_forward(scores);
    if gather(y == 0), out = out;  %if gather(y == 0), out = yp;
        grads = 0;
        return;
    end
     out1 = out.*M;  % s = mask*mixture, y clean speech signal
     [dataLoss, dscores] = softmax_backward(out1, y);
    %[dscores] = relu_backward(out, cache);
