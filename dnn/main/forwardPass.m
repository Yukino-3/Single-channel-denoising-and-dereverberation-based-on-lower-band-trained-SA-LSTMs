...
for ii = 1:num_data_layer
    if ii == 1
        net_activation = data';              % in the 1st layer, the activation is the input
        ...
        if ii == num_data_layer
            net_activation = compute_unit_activation(net_potential,opts.unit_type_output);  % calculate the output
        else
            net_activation = compute_unit_activation(net_potential,opts.unit_type_hidden);  % calculate the activation in hidden layer
        end
    end
    opts.isGPU = 0;
    if drop_flag
