function [model, pre_net] = funcDeepNetTrainNoRolling1(train_data, train_target, cv_data,cv_label, test_data, test_label,ptest_label, opts)
...
model = cv_rec(m_i*opts.cv_interval).model;   % set the model as the best model
...
% else
%     disp('wiener');
%     [a, b] = checkPerformanceOnData_save_wiener(model,model1,test_data,test_label,ptest_label,opts, 1);
% end
