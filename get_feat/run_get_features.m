...
global feat_data feat_label feat_mix;
rng default;
...
feat_data = []; feat_label=[]; feat_mix=[];
for i=1:feat_set_size
    [tmp_features, tmp_IBM,tmp_mix] = just_ibm( small_mix_cell{i}, small_speech_cell{i}, small_noise_cell{i},fun_name, NUMBER_CHANNEL,is_wiener_mask, db);
    feat_data = [feat_data; transpose(tmp_features)];
    feat_label = [feat_label; transpose(tmp_IBM)];  
    feat_mix = [feat_mix; transpose(tmp_mix)];
    fprintf(1,'index = %d\n',i);
end
