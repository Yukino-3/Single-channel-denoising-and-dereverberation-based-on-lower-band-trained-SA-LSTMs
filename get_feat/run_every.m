...
delete('*.mat');
format compact;
% Indicate the feature type and the noise type
fprintf(1,'\t%s\t%s\n',current_feature,noise_line);

feat_fun = ['my_features_' current_feature];
make_global    % select the train or test
...
if store_enable
    ...
    if part >=0	    
        save([STORE_PATH filesep save_prefix  '_' noise_type  '_' f_name  '.mat' ],'feat_data','feat_label','feat_mix','feat_set_size','feat_start','feat_end','PART','-v7.3');
        clear feat_data feat_label feat_mix feat_set_size feat_start feat_end PART
    else
        save([STORE_PATH filesep save_prefix  '_' noise_type  '_' f_name  '.mat' ],'feat_data','feat_label','feat_mix','feat_set_size','feat_start','feat_end','PART','DFI','-v7.3');
        clear feat_data feat_label feat_mix feat_set_size feat_start feat_end PART DFI
    end
end
