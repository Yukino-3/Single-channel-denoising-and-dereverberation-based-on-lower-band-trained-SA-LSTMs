clear all 
clc
format compact   % the type of output format, less space between outputs in command window
warning off;     % don't display warning information

% load configurations, includes the feature type, SNR with mixture and SS steps  
load_config

% create folders for demo

tmp_path = ['DATA' filesep];    % filesep: "\"
...
if ~exist(tmp_path,'dir'); mkdir(tmp_path); end;
TMP_DIR_STR = [pwd filesep 'DATA' filesep noise_line filesep 'tmpdir']; % pwd the current path    
copyfile('config',['DATA' filesep noise_line filesep 'config']);  % Copy the folder 'config'
...
cd(['DATA' filesep noise_line]);    % cd Change current working directory
%%% Change the mixtures to the mixtures with reverberation
